...
export const featureFlags = {
  ...
  GEOGRAPHIC_ENRICHMENT_ENABLED:
    process.env.GEOGRAPHIC_ENRICHMENT_ENABLED === 'true',
  GEOGRAPHIC_FALLBACK_ENABLED:
    process.env.GEOGRAPHIC_FALLBACK_ENABLED !== 'false',

  /* Geographic Enrichment Rollout Percentage (0-100) */
  get GEOGRAPHIC_ENRICHMENT_ROLLOUT_PCT(): number {
    const pct = parseInt(
      process.env.GEOGRAPHIC_ENRICHMENT_ROLLOUT_PCT ?? '0',
      10
    );
  ...
  get GEOGRAPHIC_ENRICHMENT_BUDGET_MS(): number {
    const budget = parseFloat(
      process.env.GEOGRAPHIC_ENRICHMENT_BUDGET_MS ?? '3'
    );
  ...
  get GEOGRAPHIC_ENRICHMENT_SUCCESS_TARGET(): number {
    const target = parseFloat(
      process.env.GEOGRAPHIC_ENRICHMENT_SUCCESS_TARGET ?? '0.95'
    );
