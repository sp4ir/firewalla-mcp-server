...
export class FirewallaMCPServer {
  ...
  private setupHandlers(): void {
    // List available tools - 24-Tool Rationalized Architecture
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          ...
          {
            name: 'search_flows',
            description: 'Search network flows with advanced query filters (e.g. "dst_port:22 bytes:>10000")',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Optional Lucene-style query string',
                },
                time_range_minutes: {
                  type: 'number',
                  minimum: 1,
                  maximum: 1440,
                  default: 60,
                  description: 'Look-back window in minutes. Default: 60 (last hour)',
                },
                limit: {
                  type: 'number',
                  minimum: 1,
          ...
          },
          {
            ...
            description: 'Quickly fetch the most recent flows (default last 60 min) with minimal parameters',
            inputSchema: {
              type: 'object',
              properties: {
                time_range_minutes: {
          ...
          {
            name: 'search_devices',
            description: 'Search devices by name, IP, MAC or status',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Free-text filter; leave blank to list all',
                },
                status: {
                  type: 'string',
          ...
          {
            name: 'search_rules',
            description: 'Search firewall rules by target, action or status',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Free-text or key:value search',
                },
                status: {
                  type: 'string',
                  enum: ['enabled', 'paused', 'any'],
                  default: 'any',
                },
                action: {
                  type: 'string',
                  enum: ['block', 'allow', 'any'],
                  default: 'any',
                },
                limit: {
                  type: 'number',
          ...
          {
            name: 'search_alarms',
            description: 'Search alarms using full-text or field filters (status, severity, geo, etc.)',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Free-text or key:value query',
                },
                status: {
                  type: 'string',
                  enum: ['open', 'acknowledged', 'deleted', 'any'],
                  default: 'any',
                },
                time_range_minutes: {
                  type: 'number',
